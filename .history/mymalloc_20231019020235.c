...
void *mymalloc(int size){
    ...
    while((char*)current < memory + MEMLENGTH)
    {
        if(current -> size == 0 && current -> alloc == FREE){ // case: if memory hasn't been initialized
            ...
            current += size+8;
            printf("size %d current before initalization \n", current->size);

            current = current + sizeof(header) + size;
            current -> size = MEMLENGTH - (size+sizeof(header));
            current -> alloc = FREE;

            printf("size %d current with remaining memory length \n", current->size);
            return res;
        }
        ...
        printf("this should only print on the second malloc %d, %d", current->size, size);
...
int main()
{
    ...
    int* arr2 = (int *) mymalloc(sizeof(int) * 8);
