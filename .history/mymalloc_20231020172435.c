...
void *mymalloc(int size){
    ...
    while (count < MEMLENGTH-8)
    {
        ...
        if (current -> hSize >= size + sizeof(header) && current -> alloc == FREE)
        {
            ...
            current = (char*)current + sizeof(header) + size;
        ...
        if (current -> alloc == ALLOCATED || current -> hSize < size + 8)
        {
            ...
            current = (char*)current + current -> hSize + sizeof(header); 
        }
    }
...
int memCleared()
{
    ...
    if ((current -> hSize == 0 && current -> alloc == FREE) || (current -> hSize == MEMLENGTH - sizeof(header) && current -> alloc == FREE))
        return 1;
    ...
    while (count < MEMLENGTH)
    {
        if (((char*)current + current -> hSize + sizeof(header) + 8) == ptr && current -> alloc == FREE)
        {
            ...
            if ((ptrData + ptrData -> hSize + sizeof(header)) -> alloc == FREE)
            {
                ...
                // coalesce adjacent free chunks
                current -> hSize = current -> hSize /*8*/ + (char*)(current /*0*/+ sizeof(header)/*8*/ + current -> hSize/*8*/) -> hSize;
